...
| FR-57 | Conversation history per call = system prompt + last `maxHistorySteps` user/assistant pairs + the current turn. A prior turn keeps the model's action verbatim but replaces its page outline with a `[page state at step N]` stub; only the current turn carries the full compact view. Retained content stays inside the data boundary (AC-P5, AC-P7).                                                                             |
