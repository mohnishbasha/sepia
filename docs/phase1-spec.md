...
| FR-58 | Conversation history per call = system prompt + last `maxHistorySteps` user/assistant pairs + the current turn. A prior turn keeps the model's action verbatim but replaces its page outline with a `[page state at step N]` stub; only the current turn carries the full compact view. Each stub after the first also reports the preceding step's action, its result code, and whether the page changed, so an action's outcome reaches the model explicitly rather than by diffing outlines it can no longer see. Retained content stays inside the data boundary (AC-P5, AC-P7). |
...
_End of Phase 1 specification. Awaiting maintainer approval before Phase 2 (Scaffolding / M0) begins._
