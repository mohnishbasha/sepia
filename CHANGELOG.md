...
- **`serve` launched a browser per request.** Every `POST /run` paid full Chromium startup and closed it in a `finally`, so throughput was bounded by process launches rather than by the model. The HTTP server now keeps a pool of warm browsers sized to `maxConcurrent`, and each request builds its own `BrowserContext` inside one — cookies, storage and cache stay per-session exactly as before, which is why the process is pooled and the context is not. Measured over 5 runs: 161 ms/run of setup down to 53 ms/run, ~67% saved, and the saving grows with request volume. `/metrics` reports `pooledBrowsers`, and the pool closes with the server. A named persistent profile still launches its own, since such a profile _is_ the browser. (AC-H1, issue #13)

- **The training export could not be run, and produced nothing useful if it were.** `exportToShareGPT()` and `exportToAlpaca()` required a `Map<runId, string[]>` of per-step page text that nothing in the codebase produced — the agent never recorded page content and `RunTrace` had no field for it — so `make export-traces` passed an empty Map and every sample came out with an empty `Current page:`: valid JSONL, worthless as training data, since the input the sample teaches from was missing. Separately, the Makefile target died with `MODULE_NOT_FOUND` before writing anything, so the documented command never ran at all. Steps now record the page the model saw (redacted and injection-sanitised) behind the opt-in `privacy.recordPageContent`, the exports read it from the trace, a step with no recorded page is skipped rather than emitted hollow, and the target runs `scripts/export-traces.mts` and reports how many samples it wrote — warning when that is zero instead of leaving an empty file that looks like success. (AC-TR1, issue #21)

- **Two config fields were declared, defaulted, documented and read by nothing.** `model.maxTokensPerStep` was bounded by `mergeConfig` while the agent hardcoded `max_tokens: 1024`, so setting it did nothing — and 1024 is tight for a reasoning model, which spends part of that budget before writing any JSON, failing in a way an operator could not configure around. It is now sent on every call, and its default drops from 100,000 to 8,192: the old value was never sent anywhere and would be rejected outright by most providers. `browser.humanTiming` had no behaviour behind it at all and is removed rather than invented — pointer and typing jitter is a fingerprinting feature that deserves its own spec, not something to improvise in a config cleanup. (AC-C1, issue #20)

- **Nearby labels attached to nothing on row-structured pages.** On Hacker News all thirty `link "hide"` came back identical, so an agent asked to hide a particular story could only guess. The disambiguating text — the story title — sits in a different table row, so it was neither sibling nor ancestor, and the one candidate the walk did find was the page name, true of all thirty: it lost the distinctness check but pre-empted the search for anything better. Candidate selection now runs per group, falling back to the nearest preceding node above the control in the flat view, which labels all thirty with their titles. Unnamed controls (HN's upvote arrows) are deliberately left alone — they are identified by what follows them, and the backward walk gets the previous row every time. Trimming now happens before distinctness is judged, so two labels cannot pass the check and then collapse into one. Costs +23% on Hacker News and nothing on a page whose controls do not collide. (AC-S12, issue #44)

- **MCP `observe` never reported element state.** The outline and the structured output carried handle/role/name/value/context and nothing else, so a host could not tell a disabled button from an enabled one — it clicked, got `ok`, and nothing happened. The engine tracked the state all along and the built-in agent rendered it. Both MCP surfaces now report `disabled`, `checked`/`unchecked`, `required`, `expanded`/`collapsed` and `selected`, omitting `enabled: true` as the default. `selected` was rendered by neither surface and is fixed in `formatNode()` too. (MCP-16, issue #43)

- **History stubs hid whether an action worked.** Collapsing prior outlines to `[page state at step N]` removed the model's only signal about its own actions: results were never in the conversation, so it inferred them by diffing consecutive outlines. A dead control could be clicked three times with the prompt reporting nothing but the model's own requests, leaving loop detection to end the run as `unable`. Each stub after the first now reports the preceding step's action, its result code, and whether the page changed — `[page state at step 1] — previous action click e1 → ok, page unchanged`. The page comparison reuses the `hashView()` already computed for loop detection. Costs 11-13 tokens per retained turn, ~1% of the saving it protects. (AC-AG12)

- **`minimal` verbosity was a no-op.** Its filter kept "nodes with a handle plus headings", which on many pages (list pages in particular) is everything `standard` already keeps — both levels produced byte-identical output, and AC-S6's old "fewer or equal" assertion passed by equality. `minimal` now emits handle-bearing nodes only and drops state that only restates the default (`{enabled: true}`). It deliberately preserves typed `value`s, non-default state (`disabled`/`checked`/`required`/`expanded`/`selected`), and `context`: hiding what a user already typed invites blind re-typing, and dropping context reintroduces issue #3's indistinguishable "Delete" buttons. On the corpus the knob now saves 13–63% of nodes and 6–56% of tokens per page. (AC-S6)
...
- **History re-sent the full page outline every turn.** Each prior entry was `Goal + entire page outline`, so turn n re-sent everything from turns 1..n−1 — measured at 192 → 3,205 prompt tokens over 13 calls (~24k total) on a form whose structure never changed. Prior turns now keep the model's action verbatim and collapse the outline to `[page state at step N]`; only the current turn carries the full compact view. Composes with `maxHistorySteps`: count-bounded turns × non-repeating outlines = bounded prompt growth. (AC-AG11)
