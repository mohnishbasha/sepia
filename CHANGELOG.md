...
- **No loop detection — repeated identical actions burned the step budget.** A model stuck on a dead element (unresponsive button, unopened modal, CAPTCHA) re-issued the same `(action, handle)` until `maxSteps` ran out. New `agent.loopThreshold` (default `3`, bounded `[2, 20]`): when the same `(action, handle)` repeats that many consecutive times with an unchanged `hashView()` of the observed view, the run stops early with `outcome: 'unable'` and a diagnostic `answer`. Pairs with issue #5's `unable` path; CLI/HTTP callers see `exit 1` / HTTP `422` with zero caller-side changes. (AC-AG10)
